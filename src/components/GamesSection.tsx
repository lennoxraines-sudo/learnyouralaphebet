...
import { Gamepad2, ChevronDown, ChevronUp, Play } from "lucide-react";
import { motion } from "framer-motion";
import carouselGaming from "@/assets/carousel-gaming.jpg";
import carouselArcade from "@/assets/carousel-arcade.jpg";
import { originalGames, driveGames, Game } from "@/data/games";
...
const GamesSection = () => {
  ...
  return (
    <section id="games" className="mx-auto max-w-6xl px-4 py-20">
      <SectionHeader title="GAMES" subtitle={`${originalGames.length + driveGames.length}+ playable games — click to play instantly`} icon={Gamepad2} />
      <ScrollFlipIn>
        <div className="mb-8">
      ...
      >
        {originalGames.map((g) => (
          <motion.div key={g.url} variants={item}>
            <ScrollZoomIn>
              <LinkCard {...g} onPlay={() => setActiveGame(g)} />
            </ScrollZoomIn>
          </motion.div>
        ))}
      </motion.div>

      {/* Playable HTML games toggle */}
      <div className="mt-8">
        <button
        ...
        >
          ...
          {showDriveGames ? "Hide" : "Show"} {driveGames.length}+ More Games
          {showDriveGames ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
        </button>
        <p className="mt-2 text-center text-xs text-muted-foreground">Click any game to play it directly on this page</p>
      </div>
        ...
        >
          {driveGames.map((g) => (
            <button
              ...
              className="flex items-center gap-2 rounded-md border border-border bg-card/30 px-3 py-2 text-left text-sm text-foreground transition-colors hover:border-primary/50 hover:bg-primary/5"
            >
